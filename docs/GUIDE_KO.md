...
| 크라켄 계정 | 본인인증(KYC) 완료 |
| 텔레그램 봇 **1개** | [@BotFather](https://t.me/BotFather)에서 생성 — 알림·거래·설정을 이 봇 하나가 전부 처리 |
...
1. 텔레그램에서 [@BotFather](https://t.me/BotFather) 검색 → `/newbot`
   (예: `mykraken_bot`) → 발급되는 `123456:ABC-...` 형태의 **토큰**을 메모
2. [@userinfobot](https://t.me/userinfobot)에게 아무 메시지나 보내면 내
   **chat id**(숫자)를 알려줍니다. 메모하세요.
3. 새로 만든 봇에게 먼저 `/start`를 한 번 보내두세요.

이 봇 하나가 매수/매도 **알림**, 잔고/수익 **대시보드**, 포지션 강제 청산 등
**거래 조작**, 그리고 **설정 변경**까지 전부 버튼으로 처리합니다.
(freqtrade 내장 텔레그램은 끈 상태로 두는 것이 정상입니다.)
...
| `TELEGRAM_CHAT_IDS` | 내 chat id (여러 명이면 쉼표로 구분) |
...
매수/매도 알림은 자동으로 도착하고(🟢 진입, ✅ 체결, 💰 청산, ⚠️ 취소),
`/start`를 보내면 버튼 메뉴가 열립니다. 모든 기능이 버튼 2~3번으로 연결되고,
목록이 길면 ◀ 1/3 ▶ 페이지 버튼이 붙습니다:

```
🏠 Home
├ 📊 Dashboard : 잔고 · 누적 수익 · 열린 포지션 · 전략 · 모의/실전 [🔄 새로고침]
├ 📈 Trades    : 열린 포지션 목록(페이지) → 상세 → ❌ 강제 청산 → ✅ 확인
├ 🛡 Risk      : 손절폭, 최대 동시 포지션, 운용 자본, 트레일링 스탑
├ 💰 Capital   : 목표 금액 / 확보 금액 / 리셋 자본 / 기능 on-off
├ 🤖 Bot       : ⏸ 일시정지 / ▶️ 재개 / 🔄 설정 리로드
├ 🔔 Alerts    : 어떤 알림을 받을지 종류별 on/off 토글
├ 📋 Failures  : 실패·취소 주문 기록 (페이지)
└ 🧾 Tax       : 세금 기록 동기화 상태
```

설정 버튼을 누르고 새 값을 메시지로 보내면 → 범위 검증 → 파일 저장 →
freqtrade에 즉시 반영(`reload_config`) → 변경 이력(audit log) 기록까지
자동입니다. 강제 청산은 반드시 ✅ 확인 버튼을 한 번 더 눌러야 실행됩니다.
...
## 9. 백테스트 랩 (전략 테스트·튜닝·비교)

전략이 3개 들어 있습니다. `.env`의 `STRATEGY=` 값으로 실거래/모의투자에
사용할 전략을 선택합니다 (변경 후 `bash scripts/start.sh` 재시작):

| 전략 | 봉 | 특징 |
|---|---|---|
| `KrakenSpotStrategy` (기본) | 1시간 | EMA 추세 + RSI 눌림목, 가장 보수적 |
| `E0V1EKraken` | 15분 | 인기 커뮤니티 전략 E0V1E를 크라켄 수수료에 맞게 개조 — 거래당 순수익 **1% 이상** 목표 (총 1.7% 미만 이익으로는 익절하지 않음) |
| `NostalgiaForInfinityX6` | 5분 | 최고 인기 커뮤니티 전략 원본 — **백테스트 비교 전용** (바이낸스 40~80종목 기준으로 설계되어 소액 크라켄 실거래에는 부적합) |

모든 백테스트 기능은 `scripts/lab.sh` 하나로 실행합니다. 수수료는 기본으로
크라켄 최악 조건(테이커 0.4%)이 적용됩니다:

```bash
# 1) 과거 데이터 내려받기 (크라켄은 캔들 720개 제한 → 체결내역 방식, 오래 걸림)
bash scripts/lab.sh download --days 180

# 2) 단일 전략 백테스트 (월별 성과 표 포함)
bash scripts/lab.sh backtest E0V1EKraken --timerange 20260101-

# 3) 전략 속성(진입/청산 값) 자동 튜닝
bash scripts/lab.sh hyperopt E0V1EKraken --epochs 60

# 4) 브라우저에서 직접 조건 바꿔가며 백테스트 (FreqUI "Backtest" 메뉴)
bash scripts/lab.sh webui     # 8081 포트, SSH 터널 방식 동일

# 5) 같은 데이터로 전략끼리 비교 → 인터랙티브 시각화 리포트
bash scripts/lab.sh compare KrakenSpotStrategy E0V1EKraken --pairs "BTC/USD"
```

비교 리포트는 `companion_data/lab_reports/latest.html`에 생성되고, 브라우저로
`http://localhost:8090/lab/latest.html`에서 볼 수 있습니다 (VPS 모드에서는
`ssh -L 8090:127.0.0.1:8090 user@vps` 터널 먼저). 리포트 내용: 누적 수익
곡선 비교, 낙폭(드로다운), 월별 수익, 개별 거래 산점도, 항목별 우승 전략
표시(✓)가 있는 지표 표. 다크/라이트 모드 지원, 오프라인에서도 열립니다.

메모리를 많이 사용하니 RAM 2GB + 스왑 환경에서는 `--pairs`로 코인 수를
줄여서 받으세요.
...
A. 네. `config/risk_settings.json` 수정 후 텔레그램 봇의
🤖 Bot → 🔄 Reload config 버튼을 누르면 반영됩니다.
