"""Companion entrypoint: webhook API + capital manager + tax logger + unified Telegram bot.
...
from .settings_bot import CompanionBot
...
async def run() -> None:
    ...
    notifier = TelegramNotifier(Env.TELEGRAM_BOT_TOKEN, Env.TELEGRAM_CHAT_IDS)
    ...
    tg_app = None
    if Env.TELEGRAM_BOT_TOKEN and Env.TELEGRAM_CHAT_IDS:
        bot = CompanionBot(Env.TELEGRAM_BOT_TOKEN, Env.TELEGRAM_CHAT_IDS,
        ...
        logger.info("Unified Telegram bot polling started")
    else:
        logger.warning("TELEGRAM_BOT_TOKEN / TELEGRAM_CHAT_IDS not set — "
